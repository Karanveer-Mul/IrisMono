...
- **An organization can require MFA.** `PUT /api/auth/organization/mfa` (ORG_ADMIN, and only once the admin has enrolled themselves). A member without a second factor then gets a session that can reach enrolment and nothing else — refusing the sign-in outright would strand them, since enrolling needs a session. Applies at each member's next sign-in; tokens already issued keep working for their remaining lifetime.
