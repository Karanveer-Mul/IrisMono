...
- ~~**Nothing validates the uploaded object.**~~ *Resolved — see "Presigned POST for real S3 uploads" in §7.* The mock path enforces a size ceiling and a PNG-signature check by streaming the body as it arrives; real S3 now gets the equivalent through the upload's own conditions rather than through code that never sees the bytes.
...
| §1 | Exactly 1 image at a time, config-driven | ✅ | Server-enforced per tenant, `organizations.max_concurrent_jobs` — see "Job concurrency" in §7 |
| §1 | Presigned URL, direct-to-storage upload | ✅ | Presigned POST with a `content-length-range` condition for real S3; the mock path enforces the equivalent by streaming — see "Presigned POST for real S3 uploads" in §7 |
| §1 | Isolated GPU worker containers via message queue | ✅ | `Dockerfile.worker`, `docker-compose.yml` — see "Containerized GPU worker" in §7. The model itself remains simulated, which was always this project's own stated scope rather than a requirement this row measures |
...
*A KMS* was listed here and the swap-in promised at the time has since been built — see "KMS-backed envelope encryption" below. The master key environment variable remains the default and is still what MFA secrets wrap under regardless; it is deliberately **not** defaulted to a literal key, for the reason given when this item was first written: a deployment that forgot to set it would otherwise encrypt everything with a key published in this repository, which looks encrypted and is worse than honest plaintext.

*SSO/SAML.* Needs an identity provider to federate with, which this project does not have. Worth noting that SSO is usually what a hospital actually asks for, and that it displaces MFA rather than complementing it — the second factor then lives in the IdP.

*MFA* was listed here alongside SSO and has since been built — see "Multi-factor authentication" below. The part of the original objection that stands is the last-resort recovery path, which remains a human process rather than code.
...
*A prefix is still not a boundary* was listed here and the IAM half has since been built — see "Per-tenant IAM for presigned uploads" below. RLS enforces the database side and per-tenant keys enforce the storage side cryptographically; what remained was the credential path itself, since the mock storage layer had no IAM at all to model against.
...
`WorkspacePanel` (ORG_ADMIN, mirroring the API's own gate) does both. The retention control names the window in days, and says explicitly whether it is the tenant's own or the platform's — so `GET /api/auth/profile` now returns `platformRetentionDays` alongside the organization. Without it the interface could only offer a choice between "14" and the word "default", which is not a choice an administrator can make. The panel also states what shortening the window does on the next sweep, because it is an instruction to delete.

Closure asks the administrator to type the workspace name. The cost of doing it by accident is every member losing access at once, and it is the one action here that a second look is worth. The confirmation text says plainly that jobs, credits, and audit records are kept and that this is not erasure, and points at the actual answer for a customer who wants the images destroyed — discarding the workspace's data key. A closed workspace still renders the dashboard, with a banner, because the administrator holding the pre-closure token is the only person who can reopen it; that the ability ends when they sign out is stated on screen rather than discovered.

Verified through the frontend origin so the Next rewrite is in the path: the profile carries the default (30) and the tenant's own window; 4000 days is refused; clearing returns `usingPlatformDefault`; closure returns its retention note and the profile still answers 200 with `deleted_at` set while a retention write against the closed workspace answers 410 and a fresh sign-in is refused; reopening succeeds once and answers 409 the second time, after which sign-in works again. A member's token is refused on both endpoints (403). `npm run test:retention` check 15 pins the profile's two retention fields.

### Multi-factor authentication (design item §2.6, MFA)

The original objection to building MFA here was that a code generator without enrolment and recovery is a checkbox, and that the recovery flow is where MFA deployments actually fail. That objection was about the surrounding parts, not about TOTP, and the surrounding parts are all implementable: TOTP (RFC 6238) needs no third party, because the second channel is the authenticator app the user already has and the shared secret never leaves the deployment.

**Enrolment is two steps, and that is the point.** `POST /api/auth/mfa/setup` writes a secret and returns it once with an `otpauth://` URI; MFA is not in force until `POST /api/auth/mfa/confirm` proves a code from it. An enrolment abandoned at the QR screen — the usual outcome when a scan does not work — therefore leaves the account exactly as it was, rather than locked out of itself.

**Codes cannot be replayed.** A TOTP code is valid for a whole 30-second step, so accepting one without recording the step leaves it usable for the rest of that window by anyone who saw it: over a shoulder, in a screen share, in a proxy log. `users.mfa_last_step` is claimed conditionally in the same statement that burns it, so a replay races and loses rather than being checked and then written. Drift is one step either side, which is what the RFC suggests and what phones need; wider would extend the life of every code, including an observed one.

**Recovery codes work exactly once.** Ten, shown once at enrolment and never again — a system that can display them twice is one where reading the database bypasses the second factor. They are claimed by a conditional `UPDATE ... WHERE used_at IS NULL`, so two requests presenting the same code cannot both succeed, and used rows are kept rather than deleted because "this account was recovered with a code on the 4th" is what an investigation needs. Hashed with SHA-256 rather than bcrypt, deliberately: bcrypt's cost exists to slow the guessing of low-entropy secrets people choose, these are 80 bits the system chose, and ten bcrypt comparisons per attempt would hand an attacker a CPU amplifier pointed at the login endpoint. Transcription is normalised — these get written on paper, and case and spacing must not decide whether someone gets back in.

**The challenge token is not a session.** The password step now returns a token that names the user, carries no organization or role, expires in five minutes, and is refused by every authenticated route. `authenticateJWT` was changed to reject *any* token carrying a `purpose` claim rather than enumerating the ones it knows about, so the next narrow credential added is refused by default rather than by remembering.

**Disabling requires a current code**, not merely a live session — otherwise a stolen session is enough to remove the control whose entire purpose is to make a stolen password insufficient, and by then the attacker is the one holding the account.

One real defect surfaced during verification, and it is the reason the throttling check was written. Login cleared the failed-attempt counter as soon as the password verified — correct when the password was the whole sign-in, and wrong the moment it became the first of two factors, because a password-then-guess-the-code loop reset the limit on every iteration and left the second factor with no rate limit at all. The counter is now cleared when the sign-in *completes*, and a wrong code counts against the same lockout as a wrong password. That matters more than it sounds: an attacker at this point already has the password, and a million six-digit codes fall quickly against an endpoint that does not count.

Sign-in audit rows now record which factors were used. "Signed in with a password" and "signed in with a password and a second factor" are different events, and an investigation that cannot tell them apart cannot say whether MFA was in force at the time.

**An organization can require it.** `organizations.require_mfa` (`PUT /api/auth/organization/mfa`, ORG_ADMIN) is what a hospital actually buys — a control only the already-careful opt into protects the wrong people. Enforcement had to solve a chicken-and-egg problem: turning the requirement on strands every member who has not enrolled, because enrolling needs a session and refusing the sign-in outright means there is never one. So a member without a second factor gets a *restricted* session — real, but able to reach nothing except MFA enrolment, and told so explicitly with `mfaEnrolmentRequired` rather than a bare 403.

That restriction is enforced in `authenticateJWT`, the one place every authenticated route already passes through, by matching the mount path. Matching on a path is uglier than a per-route flag and is the right trade here: the failure mode of the tidier design is a router somebody forgot to annotate, which fails open on exactly the accounts the policy exists to cover. The admin turning the policy on must have enrolled themselves first — not paternalism, but because whoever flips it is restricted by it at their next sign-in like everyone else, and an admin locked into enrolment-only is the first support call this feature would otherwise generate. Sessions already issued are not revoked; the policy binds at the next sign-in or organization switch, and the response says so rather than leaving it to be discovered.

**Still not code.** A user who loses both their phone and their recovery codes needs a human to let them back in. This deployment deliberately has no self-service path for that: an email reset would reduce the second factor to the strength of a mailbox, which is precisely what MFA is there to stop being sufficient.

The browser has the whole flow: a challenge screen after the password, two-step enrolment with the recovery codes shown once, a Security panel for turning it on and off, and the organization-wide requirement for admins. A restricted session is gated in `AppShell` by reading the `restricted` claim from the token rather than from the sign-in response, so a reload lands on enrolment instead of on a dashboard whose every request would be refused. There is deliberately no QR code — rendering one needs a dependency or a few hundred lines of encoder, and the manual-entry key every authenticator app accepts is the same secret.

`npm run test:mfa` covers seventeen behaviours, and generates its codes from an independent RFC 6238 implementation written straight from the specification rather than from `src/mfa.ts` — a test that produces codes with the function it is checking passes even when both are wrong. The enforcement checks follow a member who joined *before* the policy: refused a job with `mfaEnrolmentRequired`, still able to reach enrolment, and working normally the moment they finish.

### Job concurrency

`arch.md` §1: "The frontend must restrict users via configuration to uploading exactly 1 image at a time." The only thing enforcing it was `MaskUploader.tsx` refusing a file-picker selection of more than one file — which stops nothing about a second browser tab, a second member of the same workspace, or the same request replayed by hand. Nothing on the server ever counted how many jobs a tenant already had running, so the actual ceiling was whatever the credit balance allowed.

`organizations.max_concurrent_jobs` (migration `0016`) is the same shape as per-tenant retention: a platform default (`MAX_CONCURRENT_JOBS_DEFAULT`, 1) that a tenant can override upward through `PUT /api/auth/organization/concurrency` (ORG_ADMIN) — "exactly 1" is a starting configuration, not a law, and a hospital that provisioned more GPU capacity for itself is entitled to run more than one job at a time. NULL means the platform default for the reason it always does here: a tenant who has never stated a preference should track the deployment's policy rather than freeze today's number into their row.

Enforcement lives inside the same transaction that reserves the credit in `POST /api/jobs/request`, and in the same order that matters: the organization row is locked *before* the tenant's PENDING/PROCESSING jobs are counted, not after, or two concurrent requests could both count zero in-flight jobs and both pass a limit of one. Locking twice within one transaction — once here, once inside `reserveCredit` — is not two locks; Postgres holds the first. A tenant already at its limit gets `429` naming the limit it hit, so a client can tell "wait for one to finish" apart from `402` insufficient credits and `410` closed workspace, which look similar from the button a user clicked but mean different things.

`WorkspacePanel` gives an administrator the same visibility retention already has: the current limit, whether it is the tenant's own or the platform default, and a bounded input to change it — the same reasoning as `platformRetentionDays` applies to `platformMaxConcurrentJobs` in the profile response, since a tenant on the default has no other way to learn what it actually is.

`npm run test:concurrency` covers nine behaviours through the same endpoint a browser calls: a second job is refused while the first is PENDING, still refused once it moves to PROCESSING, freed once it reaches a terminal state, bounded (0, a five-figure ceiling, and a fractional limit are all refused), a tenant's own raised limit admits exactly that many concurrent jobs and refuses the next, a second tenant is unaffected by the first's limit, clearing the override restores the platform default, and the change is audited with the value it replaced.

### KMS-backed envelope encryption

§2.6 named this as the difference that mattered: "the master key is an environment variable, which means it sits in the same blast radius as the application that reads it — a process compromise gets both halves." The section also said what the current shape bought regardless: "the encryption boundary, the per-tenant key hierarchy, and the call sites are already correct, so moving to KMS is a change to two functions." That promise is now kept literally — `generateWrappedTenantKey` and `unwrapTenantKey` in `src/crypto.ts` are the two functions, and nothing else in the file, the schema, or a caller changed.

`KMS_KEY_ID` set routes a tenant's data key through AWS KMS: minted by `GenerateDataKeyCommand` rather than `randomBytes`, wrapped as the KMS ciphertext blob rather than under the local master key. Unset, wrapping is exactly what it was before this existed. Which path produced a given `wrapped_key` is decided by a magic prefix on the stored bytes, not by the current value of `KMS_KEY_ID` — a key wrapped under KMS must still unwrap through KMS after the environment variable is unset again, and a key wrapped locally before KMS was ever configured must still unwrap locally after it is. Legacy rows carry no prefix at all, which is simply what the local format has always looked like, so turning `KMS_KEY_ID` on requires rewriting nothing that already exists; only tenants provisioned afterward get KMS-wrapped keys.

Scope is deliberately the tenant key only. The scan itself stays AES-256-GCM under that unwrapped key regardless of how the key arrived — that is envelope encryption's actual mechanism, not a detail KMS changes — and MFA secrets keep wrapping under the local master key unconditionally, because they are scoped to a person who may belong to several organizations and have no single tenant's key to live under. `MASTER_KEY_BASE64` therefore stays a hard requirement in both modes.

Not exercised against a live KMS here, because nothing in this stack runs one — the same position the non-mock S3 presign branch has always been in. `KMS_ENDPOINT` exists for the same reason `AWS_S3_ENDPOINT` does: so a KMS-compatible emulator can stand in without a code change, whenever a deployment target has one.

### Per-tenant IAM for presigned uploads

The other half of "a prefix is a naming convention, not a boundary": the application authenticates to S3 with one shared, long-lived credential for every tenant, so nothing has ever bounded a presigned URL to the tenant it was issued for beyond the key string the server chose to put in it. RLS closed the database side of this; the storage side had nothing analogous, because there was no per-tenant identity to scope in the first place.

`AWS_S3_ASSUME_ROLE_ARN` set means every presigned URL in `POST /api/jobs/request` is minted from temporary credentials obtained via `AssumeRoleCommand`, carrying an inline session policy that restricts `s3:PutObject` to that one tenant's `org_id=<uuid>/*` prefix and nothing else — `scopedS3ClientFor` in `src/routes/jobs.ts`. This is the standard shape for one application identity serving many tenants: the base role only has to be broad enough to cover the bucket, and the session policy narrows it per request, so a bug in this handler cannot mint a URL that reaches past the prefix even if it tried, because the credentials behind it are incapable of it regardless of what URL is constructed. The alternative — a distinct IAM principal per tenant — does not fit an application that talks to S3 with a single shared credential to begin with. Credentials are requested fresh per call rather than cached, since they are only ever good for the one URL a single request is about to mint.

Unset, presigning uses the shared client exactly as before every call site. Like the KMS path above, this is real code against the documented STS API, exercised no more than that until a role ARN on the other end of it exists.

### Containerized GPU worker

The remaining half of the §5 row this project never closed: `worker.ts` consumed RabbitMQ correctly from the start, but ran on the host via `npm run worker:dev` like every other process here, which is not what "isolated GPU worker containers" means. Isolation is the property that made §2.2's fix matter in the first place — the worker holds no database credentials and, as of this section, no long-lived cloud credentials with tenant-wide reach either — and a process sharing the host's filesystem, network namespace, and package installation with everything else undoes it regardless of what permissions it was granted.

`Dockerfile.worker` builds and runs the same `src/worker.ts` in its own image, and the `worker` service in `docker-compose.yml` wires it to the broker over the compose network rather than `localhost`. What it does *not* get is a database connection, an S3 credential, or a KMS key — the container's environment carries exactly what `src/worker.ts` already declared it needs: `AMQP_URL`, `API_BASE_URL`, `WORKER_SECRET`, `MODEL_VERSION`. Isolation enforced by the container boundary matches isolation the code already claimed for itself; this did not require touching `worker.ts`.

The API is not itself containerized in this project — it runs on the host in every environment this document describes — so the worker container reaches it through `host.docker.internal` rather than a compose service name, and shares the mock storage directory (`../uploads`) as a volume rather than through S3 credentials, since that mock layer is what the host API process also writes through. Both are named in `Dockerfile.worker`'s and `docker-compose.yml`'s comments as what a real deployment replaces: `host.docker.internal` becomes the API's real address, and the shared volume becomes the S3 credentials `scopedS3ClientFor` already knows how to scope per tenant.

Verified against the running stack, not just built: `docker compose up -d worker` joins the same broker as the host API, answers its own `/health/ready` on `9101`, appears in `GET /health/workers` under its container hostname, and — pushed a real job through the full path (register, request, presigned upload, dispatch) — claimed it, wrote the mask to the shared volume, and reported `SUCCESS` back to the host API with its container hostname recorded as `worker_id` on the row. `npm run worker:dev` on the host remains the faster loop for iterating on the worker itself; the container is what the audit's own claim now has to survive.

The model this worker runs is still `Math.random() > 0.1` and a `sleep(5000)`, and that stays a simulation deliberately — this project's own scope was never to ship a real segmentation model, only the architecture a real one would run inside, so the §5 row now measures what it should: the container, not the model.

### Presigned POST for real S3 uploads

§2.4's last open line: "The presigned PUT sets only `ContentType` — no size ceiling, no content verification... Presigned POST with a `content-length-range` condition is the correct primitive." That recommendation is now what `POST /api/jobs/request` does for real S3, rather than staying a recommendation.

The reason a presigned PUT could never close this on its own is what it actually signs: who may write to a key, not what the body may contain. S3 does not evaluate a PUT's signature against the bytes that follow it at all, so bounding size or type through one requires the application to sit in the upload's request path — which defeats a presigned URL's entire purpose, forwarding the request through this API rather than letting the client reach storage directly. A presigned POST's `Conditions` are evaluated by S3 itself, against the multipart body as it streams in, which is the only place a check can happen without the app rejoining the path it was trying to leave.

`createPresignedPost` (`@aws-sdk/s3-presigned-post`) replaces `getSignedUrl`/`PutObjectCommand` for the real-S3 branch, carrying a `content-length-range` condition matching `MAX_UPLOAD_BYTES` and a fixed `Content-Type` condition — the same ceiling the mock path already enforced by a different mechanism (streaming the body and counting bytes as they arrive, since that path *is* the application). The response now carries `uploadFields` alongside `uploadUrl` when — and only when — the upload is a presigned POST; the mock path's response is unchanged, so `MaskUploader.tsx` branches on the field's presence rather than on any assumption about which mode is active. Dispatch is unaffected either way: it was already decoupled from the upload call itself for real S3, driven by the bucket's own event notification (see "Storage-driven dispatch" above) rather than by the upload response, which is what let the upload mechanism change without touching how a job gets queued.

Exercised the same way the KMS and per-tenant IAM paths above are: real code against the documented API, verified by `tsc` and by the request/response contract, not by a live bucket — nothing in this stack runs real S3, and the mock path remains what every test suite here actually uploads through.

**Status update.** All eight defect fixes have been applied, plus eight items from the design critique and part of a ninth — the credit ledger, job provenance, the SSE bus, the identity model, the operational surface, the queue topology's retry policy and idempotency, storage-driven dispatch, retention of record, per-tenant retention, MFA, and the implementable half of the security work — every conformance gap in §5 (the config-driven concurrency limit, the containerized GPU worker, and presigned-POST uploads for real S3), and the two remaining §2.6 items that were code rather than infrastructure or contract (the KMS swap-in and per-tenant IAM scoping for presigned uploads). What is left unbuilt in §2.6 — SSO/SAML, DICOM de-identification, BAA scope — remains so for the reasons given there: each needs something this project does not have (an identity provider, a defensible de-identification policy, a counterparty), and a convincing-looking stand-in for any of them would be worse than the honest gap. Twelve suites cover the result: `test:flow`, `test:rls`, `test:lifecycle`, `test:credits`, `test:sse`, `test:identity`, `test:observability`, `test:security`, `test:retention`, `test:mfa`, `test:concurrency`, and the ad-hoc verification described above. The backend compiles, starts, and passes `src/test-flow.ts` end to end — registration, domain whitelist enforcement, credit reservation, queue dispatch, GPU worker completion, and invite revocation all behave as specified, with the organization balance moving 3 → 2 on one successful job.
