...
**Still not code.** A user who loses both their phone and their recovery codes needs a human to let them back in. This deployment deliberately has no self-service path for that: an email reset would reduce the second factor to the strength of a mailbox, which is precisely what MFA is there to stop being sufficient. Enrolment is also not enforceable per organization yet — an admin cannot require it of their members, which is what a hospital would actually buy.

`npm run test:mfa` covers thirteen behaviours, and generates its codes from an independent RFC 6238 implementation written straight from the specification rather than from `src/mfa.ts` — a test that produces codes with the function it is checking passes even when both are wrong.
