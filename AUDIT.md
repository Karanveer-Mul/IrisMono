...
*BAA scope, and per-tenant retention.* Contractual. Worth stating that the retention sweeper is global (`STORAGE_RETENTION_DAYS`), so a hospital contract requiring seven days rather than thirty cannot be honoured today without per-prefix lifecycle rules or object tagging.
